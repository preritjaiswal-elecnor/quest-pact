...
async function hashPassword(password, salt, pepper) {
  ...
  const keyMaterial = await crypto.subtle.importKey('raw', enc.encode(password + pepper), { name: 'PBKDF2' }, false, ['deriveBits']);
...
async function getAuthStore(env) {
  ...
  const res = await fetch(url, { headers: { Authorization: `token ${env.GITHUB_TOKEN}`, Accept: 'application/vnd.github.v3+json', 'User-Agent': 'QuestPact/1.0' } });
...
async function putAuthStore(env, store, sha) {
  ...
  const body = { message: 'chore: update auth store', content: btoa(JSON.stringify(store, null, 2)), branch: env.GITHUB_BRANCH || 'main' };
  if (sha) body.sha = sha;
  await fetch(url, { method: 'PUT', headers: { Authorization: `token ${env.GITHUB_TOKEN}`, Accept: 'application/vnd.github.v3+json', 'User-Agent': 'QuestPact/1.0', 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
}
...
export async function onRequest(context) {
  ...
  const { action, playerId, password } = body;
  if (!action || !playerId) return new Response(JSON.stringify({ error: 'Missing action or playerId' }), { status: 400, headers: CORS });
  try {
    const pepper = env.AUTH_SECRET || 'default-pepper';
    const { store, sha } = await getAuthStore(env);
    if (action === 'register') {
      if (!password || password.length < 6) return new Response(JSON.stringify({ error: 'Password must be at least 6 characters' }), { status: 400, headers: CORS });
      if (store[playerId]) return new Response(JSON.stringify({ error: 'Password already set for this player' }), { status: 409, headers: CORS });
    ...
    if (action === 'login') {
      ...
      if (!record) return new Response(JSON.stringify({ error: 'No password set yet — go to Set Password tab first' }), { status: 404, headers: CORS });
